...
(define (horizontal-reducer grid n)
  (letrec ([line-reduce (lambda (line)
		       (if (null? line)
			 '()
			 (cons (apply * (take line n)) (line-reduce (drop line n)))))])
    (apply max (apply append (map line-reduce grid)))))

(define (vertical-reducer grid n)
  (if (null? grid)
    '()
    (letrec ([lines-reducer (lambda (lines)
			      (if (null? (car lines))
				'()
				(cons
